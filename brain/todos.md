...
28. [ ] Rename `prioritize` skill/task type to `schedule` — better describes what it does (scheduling, not just prioritization). Rename skill directory, frontmatter, all references in loop, mise, config, tests, docs, and brain. [[plans/28-rename-prioritize-to-schedule/overview]]
29. [ ] Queue item context passthrough — let the prioritization agent attach arbitrary context to a queue item. This context gets injected into the spawned agent's prompt, so the prioritizer can pass along reasoning, constraints, or instructions to any task it schedules. [[plans/29-queue-item-context-passthrough/overview]]

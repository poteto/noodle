...
113. [ ] Deterministic agent completion detection — replace the fragile `terminalStatus()` heuristic (exit-code + canonical event scraping in `dispatcher/process_session.go`) with a reliable completion signal that works across all providers. Three observed failure modes: (1) zombie sessions linger after work is done, blocking next stages, (2) completed work misclassified as failed (or vice versa) causing wrong retry/advance decisions, (3) premature kills while agent is still writing files or committing. Current system: `terminalStatus()` scrapes canonical events post-exit, monitor repair catches zombies, `stage_yield` overrides exit codes — all heuristic, none deterministic. Plan phase should evaluate: protocol-level mandatory `EventComplete`, structured exit-code contracts per provider, adapter-layer validation, and whether the monitor repair layer can be eliminated. Key files: `dispatcher/process_session.go` (terminalStatus), `loop/cook_watcher.go` (session watcher), `loop/cook_completion.go` (drain/handle), `loop/session_meta_repair.go` (zombie repair), `monitor/claims.go` (event claims). #needs_plan

...
All of noodle's state lives in your project's `.noodle/` directory. If it's not on disk, it doesn't exist. Your agent can natively read `.noodle/orders.json` and reschedule it according to your workflow and scheduling needs.
...
Write .noodle/orders-next.json with the orders you want to schedule next based on <my workflow>.
```

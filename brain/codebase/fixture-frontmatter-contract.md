...
- `source_hash` in `expected.md` is generated via `noodle fixtures sync`.
- `source_hash` must match a deterministic hash of fixture input files (all files under the fixture except `expected.md` / `expected.src.md`).
- Required frontmatter keys in `expected.md`: `schema_version`, `expected_failure`, `bug`, `regression`, `source_hash`.

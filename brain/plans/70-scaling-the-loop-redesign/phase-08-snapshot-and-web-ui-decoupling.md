...
**`internal/snapshot/snapshot.go`** — `LoadSnapshot()` accepts a `LoopState` argument (or the server passes it) rather than scanning `.noodle/sessions/`. The session list comes from the loop's active cook tracking + recent history. Canonical event reading for the chat panel is done lazily per-session on demand (when the user opens a specific session).

**`server/server.go` and `server/sse.go`** — The HTTP server holds a reference to the loop's exported state (or receives state updates via a channel/callback). The `/api/snapshot` endpoint serves from memory. A new `/api/sessions/{id}/events` endpoint serves canonical events for a specific session on demand. Update all `snapshot.LoadSnapshot()` call sites (`server.go:151,165`, `sse.go:134,189`).

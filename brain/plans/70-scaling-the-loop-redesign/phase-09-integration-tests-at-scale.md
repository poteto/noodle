...
6. **Shutdown quiescence test**: Start 20 sessions, initiate drain, verify `watcherWG` completes, merge queue empties, all completions and merge results are processed before exit. No goroutine leaks.
...
- Burst test: 50 completions in one cycle — verify drain returns 50 results, all stages advance to next
...
- Benchmark: cycle time at 1000 sessions — record p99 latency (don't assert hard threshold in CI — use benchmark comparison instead). Memory growth over 10,000 cycles — record and compare, don't hard-assert.

...
When a cook completes, the loop spawns a review (taster) by default. The taster reviews the work and outputs accept/reject. On accept, the loop calls the adapter's `done` script to mark the item complete. On reject, the loop respawns a cook with the taster's feedback.
...
queue item → spawn cook → cook completes → review? → spawn taster → taster accepts?
                                                                      ├── yes → adapter done script → next item
                                                                      └── no  → respawn cook with feedback
                                           (review: false)  → adapter done script → next item
...
The taster outputs a structured verdict: `{"accept": bool, "feedback": "..."}`. On accept, the loop calls the adapter's `done` script. On reject, the loop spawns a new cook with the taster's feedback prepended to the original prompt.

## Concurrency + Worktree Isolation

When `concurrency.max_cooks` > 1, the loop spawns multiple cooks in parallel. Each cook runs in its own git worktree to prevent file conflicts. The loop creates a worktree before spawning and cleans it up after completion (or merges, depending on outcome).
...
- `go test ./loop/...` — Unit tests:
  ...
  - Taster accept → adapter `done` script called
  - Taster reject → cook respawned with feedback
  - `review: false` annotation → taster skipped, adapter `done` called directly
...
### Runtime
- `noodle start` with a single todo item: writes `.noodle/mise.json`, sous chef writes `.noodle/queue.json`, loop spawns a cook, reviews, marks done

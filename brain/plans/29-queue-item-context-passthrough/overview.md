...
The prioritize agent schedules queue items with a `prompt` (full task text) and a `rationale` (why the item was scheduled). Neither field gives the prioritizer a way to pass supplemental instructions about *how* a task should be done. For example, the prioritizer might observe from `recent_history` that a previous cook failed because it forgot to run tests, or that a particular plan phase needs a careful approach due to upstream dependencies. Today it can only encode this guidance by bloating the `prompt` field or shoehorning advice into `rationale`, which muddies both fields' purpose.

A new `extra_prompt` field on QueueItem gives the prioritizer a dedicated channel for scheduling context. It gets injected into the cook's prompt under a `Scheduling context:` prefix, keeping it visually distinct from the task description and rationale.
...
- Update prioritize skill (`SKILL.md`) to document the field and its intended use
...
- Changes to how the prioritize agent decides *what* to write in the field (that's agent judgment)
- Changes to the `rationale` field's semantics

## Constraints

- Field is free-form string, not structured. The prioritizer writes natural language.
...
- [[plans/29-queue-item-context-passthrough/phase-03-update-prioritize-skill-and-schema-docs]]

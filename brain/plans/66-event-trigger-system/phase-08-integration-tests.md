...
- Lint passes
- No references to `queue-events.ndjson` or `QueueAuditEvent` remain in non-archived source code (grep verification — exclude `brain/archived_plans/` and `brain/plans/` directories)

...
- **Quality verdict gates merge:** When a stage completes and the loop would merge, it first checks `.noodle/quality/<session-id>.json`. If a verdict exists and `accept == false`, the stage is treated as failed (triggering OnFailure or order failure).

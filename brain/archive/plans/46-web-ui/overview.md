...
1. [[archive/plans/46-web-ui/phase-01-extract-snapshot-package]] — Extract snapshot types + add RemoteHost, WorktreePath, document enums
2. [[archive/plans/46-web-ui/phase-02-go-http-server-with-sse]] — HTTP server, SSE, control POST with ack, config endpoint
3. [[archive/plans/46-web-ui/phase-03-server-integration-and-browser-launch]] — Wire into `noodle start`, browser launch, embed.FS
4. [[archive/plans/46-web-ui/phase-04-typescript-project-scaffold]] — TanStack Start SPA scaffold (can parallel with 1-3)
5. [[archive/plans/46-web-ui/phase-05-shared-client-package-types-and-sse-hook]] — TS types, SSE hook, React Query, kanban column derivation (blocks 6-10)
6. [[archive/plans/46-web-ui/phase-06-feed-view]] — Kanban board with four columns + agent/queue/review cards (parallel with 7)
7. [[archive/plans/46-web-ui/phase-07-queue-view]] — Board header: title, stats, loop state, new task button (parallel with 6)
8. [[archive/plans/46-web-ui/phase-08-reviews-view]] — Review actions: merge/reject/request-changes in Review column (parallel with 9, 10)
9. [[archive/plans/46-web-ui/phase-09-session-detail-view]] — Slack-style chat panel with event stream, steer input, @mention (parallel with 8, 10)
10. [[archive/plans/46-web-ui/phase-10-controls-steer-pause-task-editor]] — Pause/resume, task editor modal, keyboard shortcuts (parallel with 8, 9)

...
name: skill-creator
description: >-
  Guide for creating or updating skills — specialized knowledge, workflows, and tool integrations
  packaged for Claude.
---
...
Skills live in `.agents/skills/<skill-name>/`. The `.claude/skills` directory is a symlink to `.agents/skills`, so creating a skill in `.agents/skills/` automatically makes it visible to Claude.

When creating a new skill, just create the directory:

```bash
mkdir -p .agents/skills/<skill-name>
# ... create SKILL.md and any resources ...
```

If an `init_skill.py` script exists in the skill-creator's `scripts/` directory, use it with `--path .agents/skills/` as the output directory.

**Known bug:** The init script may create `skills/name/name/SKILL.md` (one level too deep). After creating a skill, check for and fix double-nesting.
...
3. Before updating SKILL.md with a new instruction, ask: can this be a script in `scripts/`, a validation check in `package_skill.py`, or a metadata flag? Structural fixes are more durable than textual ones.

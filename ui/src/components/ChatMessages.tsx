    ...
    case "think": {
      return (
        <div className="px-3 py-2 text-[0.8125rem] leading-normal bg-bg-2 border-l-[3px] border-border">
          <span className="font-mono text-xs font-semibold block mb-0.5 text-text-0">
            {event.label}
          </span>
    ...
    case "tools": {
      return (
        <div className="px-3 py-2 text-[0.8125rem] leading-normal bg-bg-2 border-l-[3px] border-nblue">
          <span className="font-mono text-xs font-semibold block mb-0.5 text-nblue">
            {event.label}
          </span>
    ...
    case "all": {
      return (
        <div className="px-3 py-2 text-[0.8125rem] leading-normal">
          <span className="font-mono text-xs font-semibold block mb-0.5 text-text-2">
            {event.label}
          </span>
          <div className="text-text-1 break-words">
            <HighlightedBody body={event.body} />
...
export function ChatMessages({ events }: { events: EventLine[] }) {
  ...
  return (
    ...
    >
      {events.map((event) => (
        <MessageBubble key={event.at} event={event} />
      ))}
      {events.length === 0 && (
        <div className="text-text-3 font-mono text-[0.8125rem] text-center pt-10">
          No events yet.
        </div>
      )}
    </div>

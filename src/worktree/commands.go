...
func (a *App) Create(name string) error {
	...
	info(fmt.Sprintf("Creating worktree at %s...", wtPath))
	...
	if _, err := os.Stat(settingsPath); err == nil {
		...
		info("Symlinked .claude/settings.local.json")
	}
...
func (a *App) Merge(name string) error {
	...
	if commits == 0 {
		info(fmt.Sprintf("No commits on '%s' — nothing to merge", name))
		info(fmt.Sprintf("Use '%s cleanup %s' to remove without merging", a.cmdName(), name))
		return nil
	}
	info(fmt.Sprintf("%d commit(s) to merge", commits))

	stashed := false
	unstaged := a.git("-C", wtPath, "diff", "--quiet").Run() != nil
	staged := a.git("-C", wtPath, "diff", "--cached", "--quiet").Run() != nil
	if unstaged || staged {
		info("Stashing worktree changes before rebase...")
		if a.git("-C", wtPath, "stash", "--include-untracked", "--quiet").Run() == nil {
			stashed = true
		}
	}

	info(fmt.Sprintf("Rebasing %s onto %s...", name, base))
	...
	info(fmt.Sprintf("Merging %s into %s...", mergeBranch, base))
	...
	info(fmt.Sprintf("Deleting branch %s...", mergeBranch))
...
func (a *App) Cleanup(name string, force bool) error {
	...
	if _, err := os.Stat(wtPath); os.IsNotExist(err) {
		if !force {
			...
			if commits > 0 {
				...
				fmt.Fprintf(os.Stderr, "WARNING: %s has %d unmerged commit(s):\n%s\n", name, commits, out)
				return fmt.Errorf(
		...
		info("Worktree directory already removed, cleaning up refs...")
		warnings := []string{}
		...
		fmt.Printf("Done. Cleaned up refs for %s.\n", name)
		return nil
	}
	...
	info(fmt.Sprintf("Deleting branch %s...", cleanupBranch))
	if a.git("branch", "-d", cleanupBranch).Run() != nil {
		if err := a.git("branch", "-D", cleanupBranch).Run(); err != nil {
			warnings = append(warnings, fmt.Sprintf("failed to delete branch %s: %v", cleanupBranch, err))
		}
...
func (a *App) List() error {
	...
	if len(names) == 0 {
		info("No managed worktrees under .worktrees/")
		return nil
	}
	for _, name := range names {
		commits, equivalent, statusErr := a.cherryStatus(name)
		if statusErr != nil {
			info(fmt.Sprintf("%s — status unknown (%v)", name, statusErr))
			continue
		}
		if commits == 0 {
			if equivalent > 0 {
				info(fmt.Sprintf("%s — patch-equivalent to %s (safe to clean up)", name, base))
			} else {
				info(fmt.Sprintf("%s — no commits ahead of %s (safe to clean up)", name, base))
			}
		} else {
			info(fmt.Sprintf("%s — %d unmerged commit(s)", name, commits))
		}
	}
	return nil
}
...
func (a *App) Prune() error {
	...
	for _, name := range names {
		...
		if commits > 0 {
			info(fmt.Sprintf("%s — %d unmerged commit(s), skipping", name, commits))
			skipped++
			continue
		}
		...
		if !clean {
			info(fmt.Sprintf("%s — patch-equivalent but dirty, skipping", name))
			skipped++
			continue
		}

		if equivalent > 0 {
			info(fmt.Sprintf("%s — patch-equivalent to %s, pruning", name, base))
		} else {
			info(fmt.Sprintf("%s — no commits ahead of %s, pruning", name, base))
		}

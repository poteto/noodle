...
Skills drive everything in Noodle. Every behavior an agent can perform is a skill: scheduling work, executing tasks, reviewing code, learning from mistakes. There are no plugins, no hooks, no configuration DSL. You extend Noodle by writing skills, and skills are just markdown files.
...
If you've used React, think of skills the way you think of components. Components are React's single abstraction. You build everything out of them, compose them together, and the framework handles the rest. Skills work the same way in Noodle. Every behavior your agents have comes from a skill. Every workflow you build is a composition of skills. You don't need to learn a second concept.
...
- **reflect** runs after each session. The agent writes down what worked, what broke, what it would do differently. Those notes go into the brain (more on that in a second).

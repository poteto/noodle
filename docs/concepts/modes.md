...
The default. Agents merge their own work. No human in the loop. The scheduling agent runs continuously, cooks execute and merge without waiting for approval.
...
The mode is a trust dial, not a capability switch. All three modes use the same scheduling loop, the same skills, the same agents. The only difference is what happens after the work is done.
...
- **Chef** (scheduling agent) — reads the mise, decides what to cook, writes orders
- **Cooks** (execution agents) — pick up orders and do the work, each in their own worktree

...
func TestMergeCookWorktreeUsesRemoteBranchSyncResult(t *testing.T) {
	...
	sess := &fakeSession{id: sessionID, status: "completed", done: make(chan struct{})}
	cook := &cookHandle{
		orderID:      "42",
...
func TestMergeCookWorktreeFallsBackToLocalMerge(t *testing.T) {
	...
	sess := &fakeSession{id: "", status: "completed", done: make(chan struct{})}
	cook := &cookHandle{
		orderID:      "42",
		stage:        Stage{TaskKey: "execute", Skill: "execute", Provider: "claude", Model: "claude-sonnet-4-6"},
		session:      sess,
...
func TestApprovalApproveCanMergeTrueParks(t *testing.T) {
	...
	if len(items) != 1 || items[0].OrderID != "42" {
